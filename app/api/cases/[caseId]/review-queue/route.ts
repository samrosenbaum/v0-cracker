...
export async function GET(
  ...
  try {
    ...
    console.log(
      `[Review Queue API] Fetching review queue for case ${caseId}, status: ${status}`,
    );
    ...
    const stats = {
      ...
      inReview:
        reviewQueue?.filter((r) => r.status === 'in_review').length || 0,
      completed:
        reviewQueue?.filter((r) => r.status === 'completed').length || 0,
      totalUncertainSegments:
        reviewQueue?.reduce(
          (sum, r) =>
            sum +
            (Array.isArray(r.uncertain_segments)
              ? r.uncertain_segments.length
              : 0),
          0,
        ) || 0,
    };
    ...
    console.log(
      `[Review Queue API] Found ${reviewQueue?.length || 0} review items`,
    );
