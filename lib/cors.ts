...
export function resolveAllowedOrigin(request: NextRequest): string {
  ...
  // Allow direct matches against explicit environment configuration
  if (ENV_ALLOWED_ORIGINS.some(allowed => allowed === origin)) {
    return origin;
  }

  // Fallback to wildcard but log for visibility so we can explicitly add it later
  console.warn('[CORS] Falling back to wildcard for unrecognised origin', origin);
  return '*';
}
